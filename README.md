...
## Remote training with Modal (canonical PyTorch flow)

This repository's canonical trainer is the PyTorch supervised trainer located at `src/training/supervised_training.py`.
Use the Modal entrypoint `src/training/train_cli.py` to run the canonical trainer remotely — helper scripts under `chessbot/scripts/` wrap the common `modal run` commands.

Quick steps (from the project root) — make sure you have a Python virtualenv active:

```bash
# Install deps (includes `modal`)
pip install -r chessbot/requirements.txt

# Make helper scripts executable (one-time)
chmod +x chessbot/scripts/*.sh

# Log in to Modal (opens browser)
modal login

# Upload your PGN to the Modal volume (used by the PyTorch trainer)
chessbot/scripts/upload_pgn_modal.sh

# Start remote PyTorch training (uses `src/training/train_cli.py`)
chessbot/scripts/train_pytorch_modal.sh

# Download the best checkpoint produced by the run
chessbot/scripts/download_best_model_modal.sh
```

Notes:

- The canonical trainer expects PGN input and uses `PGNDataset` (precompute optional) so upload a PGN to the volume before training.
- The helper scripts are small wrappers around `modal run` and point to `src/training/train_cli.py`.
- If you prefer to use a different data format (e.g. preprocessed `FEN|uci` lines or a CSV of engine evaluations), we can add a thin adapter, but keeping the PGN -> `PGNDataset` flow ensures tight integration with the rest of the repo.
- If you want, I can (A) add a short `chessbot/scripts/README.md`, (B) add a Makefile, or (C) add a GitHub Actions workflow to invoke Modal (requires secrets). Let me know which.
